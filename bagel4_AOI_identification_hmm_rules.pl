...
sub hmmsearch {
	...
	# HMMER 3.3+ has a hard limit of 100K aa per sequence.
	# Split six_frames sequences into overlapping 90K-aa chunks.
	my $CHUNK_SIZE  = 90000 ;
	my $CHUNK_OVER  = 5000  ;  # overlap to avoid missing hits at boundaries
	my $chunked_file = "$domtbloutDir/${queryname}.six_frames.chunked.faa" ;
	my %chunk_offsets ; # chunk_header => original_frame, offset
	{
		my %frames = anne_genomics::fasta2hash($protein_fasta_file) ;
		open(my $cfh, '>', $chunked_file) or die "Cannot write $chunked_file: $!" ;
		foreach my $frame (sort keys %frames) {
			my $seq = $frames{$frame} ;
			my $len = length($seq) ;
			if ($len <= $CHUNK_SIZE) {
				print $cfh ">$frame\n$seq\n" ;
				$chunk_offsets{$frame} = { frame => $frame, offset => 0 } ;
			} else {
				my $pos = 0 ; my $chunk_n = 0 ;
				while ($pos < $len) {
					$chunk_n++ ;
					my $chunk_seq = substr($seq, $pos, $CHUNK_SIZE) ;
					my $chunk_name = "${frame}_chunk${chunk_n}" ;
					print $cfh ">$chunk_name\n$chunk_seq\n" ;
					$chunk_offsets{$chunk_name} = { frame => $frame, offset => $pos } ;
					$pos += ($CHUNK_SIZE - $CHUNK_OVER) ;
				}
			}
		}
		close $cfh ;
	...
	my $hmm_folder = $conf{HMMs_folder} ;
	$hmm_folder =~ s/\s+$// ;  # strip trailing whitespace (original conf key has trailing tab)

	foreach my $pfam ( @HMMs ) {
		print "Screening for $pfam\n" ;
		my $domtblout = "$domtbloutDir/$queryname.$pfam.domtblout";
		my $tmp = "$conf{hmmsearch}/hmmsearch --noali --cpu $cpu --domtblout $domtblout --domT $domT $program_dir/$hmm_folder/$pfam.hmm $chunked_file >>$domtbloutDir/logfile.txt" ;
		push @log, $tmp ;
		system($tmp) ;
		push (@result_files, $domtblout);

		# Remap chunk coordinates back to original frame positions
		if (-e $domtblout && -s $domtblout) {
			my @lines = anne_files::read_lines($domtblout) ;
			my @fixed ;
			foreach my $line (@lines) {
				if ($line =~ /^#/) { push @fixed, $line ; next ; }
				my @f = split /\s+/, $line ;
				if (@f > 20 && exists $chunk_offsets{$f[0]}) {
					my $offset = $chunk_offsets{$f[0]}{offset} ;
					my $orig_frame = $chunk_offsets{$f[0]}{frame} ;
					$f[17] = $f[17] + $offset ;   # ali from
					$f[18] = $f[18] + $offset ;   # ali to
					$f[0]  = $orig_frame ;          # restore original frame name
					$line  = join(' ', @f) ;
				}
				push @fixed, $line ;
			}
			anne_files::write_lines($domtblout, @fixed) ;
		}
	}

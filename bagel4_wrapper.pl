...
# Create session dir FIRST so conf2json can write to it
mkdir $sessiondir ;
...
# 1. Prepare the query folder
	...
	if ($query =~ m/WGS\:(GCF_.*)\=/) { # Input is a WGS file and should be downloaded from NCBI
		...
		# Standalone mode: use $query directly as file or directory
		my @all_filenames ;
		if (-f $query) {
			# Single file passed with -query
			@all_filenames = ($query) ;
		} else {
			# Directory: scan recursively
			@all_filenames = anne_files::get_files_from_subdirs_v2($query, $regex) ;
		}
...
# 2. Scan the files and analyse them using the BAGEL pipeline
	...
	my $debug = 0 ;  # 0 = execute commands; 1 = print only (debug mode)
	# Note: originally debug=1 was the default (webserver-only execution),
	# but for standalone use we always want to execute.
	...
	foreach my $key_db (sort keys %filenames_db) {
		...
		system_command("$program_dir/bagel4_AOI_identification_blast_bacteriocins.pl -s $sessiondir -translate true -dna $sessiondir/$queryname.fna -queryname $queryname") ;
	 
		# combine overlapping AOI's from hmmsearch and blast: AOI.blast.table AND AOI.hmm.table ==> $queryname.AOI.table
		system_command("$program_dir/bagel4_AOI_merge.pl -s $sessiondir -dna $sessiondir/$queryname.fna -queryname $queryname") ;
		...
		if (-z "$sessiondir/$queryname.AOI.table") { 
			...
			system_command("$program_dir/bagel4_AOI_annotation.pl -s $sessiondir -model false -dna $sessiondir/$queryname.AOI.fna -queryname $queryname")	;
			...
			system_command("$program_dir/bagel4_promoterprediction.pl -s $sessiondir -dna $sessiondir/$queryname.AOI.fna") ;
			...
			system_command("$program_dir/bagel4_transterm.pl -s $sessiondir -dna $sessiondir/$queryname.AOI.fna") ;  # depends on .ptt file
			...
			system_command("$program_dir/bagel4_promoter_term_2_json.pl -s $sessiondir -queryname $queryname") ;

			# JSON for Genes. make a json of the multifasta. Get the AOI's names from the fasta file headers
			system_command("$program_dir/bagel4_GeneTable_2_json.pl -s $sessiondir -table $queryname.GeneTables") ;
		}
	}
...
system_command("$program_dir/bagel4_clean_tmpfiles.pl -s $sessiondir") ;
